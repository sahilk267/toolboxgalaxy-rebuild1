...
- [x] Define a transparent local-only weekly activity model that counts only completed genuine Mini Sudoku, Tango, Queens, Patches, Zip, and Wend date-edition fields; never infer a completion from opening a route, a demo, or a visual transform.
...
## Wend Seven-Edition Word-Path Bank

- [x] Play the current Wend field in a browser and audit its word-path exact-cover solver, word/clue contract, route UI, date transform, completion key, and existing authored field before changing it.
- [x] Define a seven-edition 5×5 Wend schema with independently authored target-word sets, orthogonal non-repeating exact-cover paths, difficulty bands, local-date selection, and edition-specific completion keys.
- [x] Author seven non-equivalent target-word/path fields; prove every target path is orthogonal, each 25-cell cover is complete, all word and path signatures are distinct, and no edition is only a reflection, rotation, palette change, or opening-assist variation.
- [x] Integrate the selected word set, named edition/difficulty HUD, solution paths, demo, Hint, Reset, live validation, visible word chips, and local completion persistence into the direct full-screen route.
...
- [x] Audit Image Resizer/Compressor, Favicon Generator, and Image Crop / Rotate / Convert to define a distinct Image Metadata Remover contract.
- [x] Define a privacy-safe visitor-selected-image re-export flow, including supported formats, current-tab decode/canvas output, file/pixel bounds, quality/output behavior, and clear metadata limitations.
- [x] Add one accessible browser-local Image Metadata Remover workspace with selected-image details, explicit clean re-export, download, reset, error, and no-persistence behavior.
- [x] Add deterministic coverage and real-browser tests for selected image input, clean re-export, output format/geometry, keyboard/pointer controls, download, reset, invalid input, mobile layout, and no persistence/network requests.
- [x] Update tool contracts, plan, architecture, privacy, Hostinger QA, research/roadmap, and deployment documentation; run the complete static release gate before one milestone checkpoint.
